...
class commonLib:
    ...
    def create_protocol(self):
        ...
        """
        try:
            command = [ CARDANO_CLI, "shelley", "query", "protocol-parameters", "--testnet-magic", str(TESTNET_MAGIC), "--out-file",
                        self.files['configs']['protocol']]
            s = subprocess.check_output(command)
        except:
    ...
    def calculate_min_fees(self, tx_in_count, ttl):
        try:
            command = [CARDANO_CLI, 'shelley',  'transaction', 'calculate-min-fee',  '--tx-in-count',  str(tx_in_count),
                       '--tx-out-count', str(1) , '--ttl',  str(ttl),  '--testnet-magic', str(42), 
                       '--signing-key-file', self.files['payment']['sign_key'], '--signing-key-file', self.files['stake']['sign_key'],
                       '--certificate-file', self.files['stake']['cert'],
                       '--protocol-params-file', self.files['configs']['protocol'] ]
            s = subprocess.check_output(command)
    ...
    def get_payment_utx0(self):
        ...
        try:
            ...
            command=[CARDANO_CLI, 'shelley' , 'query', 'utxo', '--address', content(self.files['payment']['addr']),
                     '--testnet-magic', '42']
            s = subprocess.check_output(command)
            split_str=s.decode('UTF-8').split("\n")
    ...
    def get_total_fund_in_utx0(self):
        t = get_payment_utx0()
        total_fund = 0
        for val in t:
    ...
    def get_deposit_fee(self):
        ...
        import json
        s = json.loads(content(self.files['configs']['protocol']))    
    ...
    def get_funds_via_faucet(self):
        ...
        try:
            import requests
            paddr = content(self.files['payment']['addr'])
...
class RegisterStake:
    ...
    def build_transaction(self, txArray, remaining_fund, ttl, min_fee):
        ...
        try:            
            ...
            payment_addr = content(self.files['payment']['addr'])
            ...
            command = [CARDANO_CLI, "shelley", "transaction", "build-raw", "--tx-out",tx_out,"--ttl", ttl, "--fee", min_fee,
                       "--out-file", self.files['transaction']['raw'], '--certificate-file', self.files['stake']['cert']] + tx_in_array
            print(command)
            s = subprocess.check_output(command)
            split_str=s.decode('UTF-8').split(" ")
        except:
    ...
    def sign_transaction(self):
        ...
        """
        try:
            command = [CARDANO_CLI, "shelley", "transaction", "sign", "--tx-body-file", self.files['transaction']['raw'], '--signing-key-file',
                       self.files['payment']['sign_key'], '--signing-key-file', self.files['stake']['sign_key'], '--testnet-magic', '42','--out-file',
                       self.files['transaction']['signed']]
            s = subprocess.check_output(command)
            print(s)
        except:
    ...
    def submit_transaction(self):
        ...
        """
        try:
            command = [CARDANO_CLI, "shelley", "transaction", "submit", "--tx-file",
                       self.files['transaction']['signed'], '--testnet-magic', "42"]
            s = subprocess.check_output(command)
