...
send_req()
{
    command="curl -X post localhost:5000/nft -d assetName=$ASSETNAME -d assetAmount=$ASSETAMOUNT -d mintingCost=$DEFAULTCOST -d recvAddr=$DEFAULTADDR -d url=$DEFAULTURL"
